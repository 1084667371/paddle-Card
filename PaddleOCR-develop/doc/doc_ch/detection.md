...
比如：
```
python tools/eval.py -c configs/det/det_mv3_db.yml  -o Global.checkpoints="./output/det_db/iter_epoch_4" PostProcess.box_thresh=0.6 PostProcess.unclip_ratio=1.5
```
...
测试单张图像的检测效果
```
python tools/infer_det.py -c configs/det/det_mv3_db.yml -o TestReader.infer_img="./doc/imgs_en/1.png" Global.checkpoints="./output/det_db/iter_epoch_1150"
```

测试DB模型时，调整后处理阈值，
```
python tools/infer_det.py -c configs/det/det_mv3_db.yml -o TestReader.infer_img="./doc/imgs_en/img_10.jpg" Global.checkpoints="./output/det_db/best_accuracy" PostProcess.box_thresh=0.6 PostProcess.unclip_ratio=1.5
```
